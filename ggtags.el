...
;; (https://www.gnu.org/software/global) within Emacs.
;;
;; Usage:
;;
;; `ggtags' is similar to the standard `etags' package.  These keys
;; `M-.', `M-?' and `C-M-.' should work as expected in `ggtags-mode'.
;; See the README in https://github.com/dalugm/ggtags for more details.
;;
...
(eval-when-compile
  ...
  (defmacro with-display-buffer-no-window (&rest body)
    "Do not create new window while executing BODY."
    (declare (debug t) (indent 0))
    ;; See https://debbugs.gnu.org/13594
...
GTAGSROOT will always be expanded to current project root directory.
This is intended for project-wise ggtags-specific process environment
settings.  Note on remote hosts (e.g. tramp) directory local variables
is not enabled by default per `enable-remote-dir-locals' (which see)."
...
(defcustom ggtags-global-window-height 8
...
(defmacro ggtags-with-temp-message (message &rest body)
  "Insert temporary MESSAGE to minibuffer while executing BODY."
...
(defmacro ggtags-ensure-global-buffer (&rest body)
  "Make sure global buffer existed while executing BODY."
  (declare (debug t) (indent 0))
  `(progn
...
(defun ggtags-program-path (name)
  "Get execution route of program NAME."
...
(defun ggtags-process-string (program &rest args)
  "Return a string of process PROGRAM with ARGS."
  (with-temp-buffer
...
(defun ggtags-make-project (root)
  ...
  (let* ((default-directory (file-name-as-directory root))
    ...
    (pcase (nthcdr 5 (file-attributes (expand-file-name "GTAGS" dbdir)))
      (`(,mtime ,_ ,tag-size . ,_)
        (let* ((rtags-size (nth 7 (file-attributes
                                   (expand-file-name "GRTAGS"
                                                     dbdir))))
               (has-refs
                (when rtags-size
               ...
               (has-path-style
                (and (ggtags-process-succeed-p "global"
                                               "--path-style"
                                               "shorter"
                                               "--help")
                     'has-path-style))
               ;; http://thread.gmane.org/gmane.comp.gnu.global.bugs/1542
               (has-color (and (ggtags-process-succeed-p "global"
                                                         "--color"
                                                         "--help")
                               'has-color)))
...
(defun ggtags-project-expired-p (project)
  "Return non-nil if PROJECT is not expired."
...
(defun ggtags-project-oversize-p (&optional project)
  "Return non-nil is PROJECT is not oversized."
...
It is unsafe to cache variable `ggtags-project-root' in non-file
buffers whose `default-directory' can often change."
      ...
      (setq ggtags-project-root
            (or (ignore-errors
                ...
                (let ((dir (locate-dominating-file
                            ...
                            (lambda (dir)
                              (file-regular-p
                               (expand-file-name "GTAGS" dir))))))
                  ;; `file-truename' may strip the trailing '/' on
                  ;; remote hosts, see https://debbugs.gnu.org/16851
...
(defun ggtags-ensure-project ()
  "Ensure current project has GTAGS created."
...
(defun ggtags-save-project-settings (&optional noconfirm)
  "Save GNU Global's specific environment variables whether NOCONFIRM."
  (interactive "P")
  (ggtags-check-project)
  (let* ((inhibit-read-only t)          ; for `add-dir-local-variable'
         ...
         (process-environment
          (append ggtags-process-environment
                  process-environment
                  (and (not (ggtags-project-has-refs
                             (ggtags-find-project)))
...
(defun ggtags-toggle-project-read-only ()
  "Toggle read-only in current project."
  (interactive)
  (ggtags-check-project)
  (let ((inhibit-read-only t)           ; for `add-dir-local-variable'
...
(defmacro ggtags-with-current-project (&rest body)
  ...
  (let ((gtagsroot (make-symbol "-gtagsroot-"))
    ...
    `(let* ((,root ggtags-project-root)
            ...
            (process-environment
             (append (let ((process-environment
                            (copy-sequence process-environment)))
                       (and ,gtagsroot (setenv "GTAGSROOT" ,gtagsroot))
                       (mapcar #'substitute-env-vars
                               ggtags-process-environment))
                     process-environment
                     (and ,gtagsroot (list (concat "GTAGSROOT="
                                                   ,gtagsroot)))
                     (and (ggtags-find-project)
                          (not (ggtags-project-has-refs
                                (ggtags-find-project)))
                          (list "GTAGSLABEL=ctags")))))
      ...
      (let ((conf (and ggtags-use-project-gtagsconf
        ...
        (unless (or conf (getenv "GTAGSLABEL")
                    (setenv "GTAGSLABEL"
                      (completing-read
                        "Select a backend: "
                        '(("default" 1) ("native" 2) ("user" 3)
                          ("ctags" 4) ("new-ctags" 5)
                          ("pygments" 6) ("native-pygments" 7))))))
...
(defun ggtags-explain-tags ()
  ...
  (interactive (ignore (ggtags-check-project)
                       (or (ggtags-process-succeed-p "gtags"
                                                     "--explain"
                                                     "--help")
...
(defun ggtags-update-tags-finish (&optional buffer reason)
  "Call when update tags file finished.
Show return REASON message in BUFFER."
  (if (and reason buffer (string-prefix-p "exited abnormally" reason))
...
(defun ggtags-update-tags-single (file &optional nowait)
  "Update gtags file for single FILE.
NOWAIT is ignored if file is remote file.
See `tramp-sh-handle-process-file'."
...
(defun ggtags-delete-tags ()
  "Delete GTAGS, GRTAGS, GPATH, ID etc files which generated by gtags."
  (interactive (ignore (ggtags-check-project)))
                         ...
                         (apply
                           ...
                           (append
                            (and completion-ignore-case
                                 ...
                                 (list (concat "-c" ggtags-completion-flag)
                                       prefix)))
                         "\n" t)))))))
...
(defun ggtags-read-tag (&optional type confirm prompt require-match default)
  "Read gtags genrated files according to TYPE and REQUIRE-MATCH.
When CONFIRM, set `ggtags-current-tag-name' according to PROMPT and DEFAULT."
          ...
          (cond
            (confirm
              ...
              (let ((completing-read-function
                ...
                (completing-read
                 (format (if default "%s (default %s): " "%s: ")
                         prompt default)
...
(defun ggtags-sort-by-nearness-p (&optional start-location)
  "Sort results according to START-LOCATION's nearness."
...
And CMD can be followed with ARGS."
  (let ((xs
         (append
          (list (shell-quote-argument (ggtags-program-path "global"))
                "-v"
                ...
                (pcase cmd
                 ((pred stringp) cmd)
                 (`definition nil)      ; `-d' not supported by Global v5.7.1
                 (`reference "--reference")
                 (`symbol "--symbol")
                 (`path "--path")
                 (`grep "--grep")
...
;; Can be three values: `nil', `t' and `marker'. t means start marker has
...
(defun ggtags-global-save-start-marker ()
  "Save current marker as start marker."
...
(defun ggtags-global-start (command &optional directory)
  "Start COMMAND under current DIRECTORY or project root."
...
(defun ggtags-find-tag (cmd &rest args)
  "Find tag by running CMD with ARGS."
  (ggtags-check-project)
...
definition tags as Do WHAT I Mean."
  (interactive
...
(defun ggtags-find-tag-mouse (event)
  "Find tag when trigger mouse EVENT."
  (interactive "e")
...
(defun ggtags-find-definition (name)
  "Find the definition tag of the NAME at point."
...
(defun ggtags-setup-libpath-search (type name)
  "Setup GTAGSLIBPATH for gtags according to TYPE and NAME."
...
(defun ggtags-find-reference (name)
  "Find the reference tag to the NAME at point."
...
INVERT the match when called with a prefix arg \\[universal-argument]."
  (interactive (list (ggtags-read-tag 'definition 'confirm
                                      (if current-prefix-arg
                                        ...
                                        "Grep pattern"))
                     current-prefix-arg))
  (ggtags-find-tag 'grep (and invert "--invert-match")
                   "--" (ggtags-quote-pattern pattern)))

(defun ggtags-find-file (pattern &optional invert)
  "Find file matching PATTERN.
INVERT the match when called with a prefix arg \\[universal-argument]."
  (interactive (list (ggtags-read-tag 'path 'confirm
                                      (if current-prefix-arg
  ...
  (let ((ggtags-global-output-format 'path))
    (ggtags-find-tag 'path (and invert "--invert-match")
                     "--" (ggtags-quote-pattern pattern))))
...
When called interactively with a prefix, ask for the directory."
  ...
  (ggtags-check-project)
  (ggtags-global-start
   (ggtags-global-build-command nil nil "-l" "--"
                                (ggtags-quote-pattern regexp))
...
(defun ggtags-global-normalize-command (cmd)
  "Normalize global CMD."
  (if (string-match
...
(defun ggtags-global-search-id (cmd directory)
  "Return sha1 with DIRECTORY as id of gtags search CMD."
  (sha1 (concat directory (make-string 1 0)
                (ggtags-global-normalize-command cmd))))

(defun ggtags-global-current-search ()
  "."
  (ggtags-ensure-global-buffer
    (list (ggtags-global-normalize-command (car compilation-arguments))
          default-directory
          ggtags-process-environment
          (line-number-at-pos)
...
(defun ggtags-global-rerun-search (data)
  "Rerun gtags search based on DATA."
  (pcase data
...
(defvar ggtags-view-search-history-mode-map
  (let ((keymap (make-sparse-keymap)))
    (define-key keymap (kbd "p") #'ggtags-view-search-history-prev)
    (define-key keymap (kbd "M-p") #'ggtags-view-search-history-prev)
    (define-key keymap (kbd "n") #'ggtags-view-search-history-next)
    (define-key keymap (kbd "M-n") #'ggtags-view-search-history-next)
    (define-key keymap (kbd "C-k") #'ggtags-view-search-history-kill)
    (define-key keymap [remap yank] (lambda (&optional arg)
                                      (interactive "P") (yank arg)))
    (define-key keymap (kbd "C-c C-c") #'ggtags-view-search-history-update)
    (define-key keymap (kbd "r") #'ggtags-save-to-register)
    (define-key keymap [return] #'ggtags-view-search-history-action)
    (define-key keymap (kbd "q") #'ggtags-kill-window)
    keymap))

(defun ggtags-view-search-history-remember ()
  "Remember last viewed search history."
...
(defun ggtags-view-search-history-next (&optional arg)
  "Go to next gtags search history item.

When with ARG, go to next ARG item(s)."
  (interactive "p")
  (let ((arg (or arg 1)))
...
(defun ggtags-view-search-history-prev (&optional arg)
  "Go to previous gtags search history item.

When with ARG, go to previous ARG item(s)."
  (interactive "p")
  (ggtags-view-search-history-next (- (or arg 1))))

(defun ggtags-view-search-history-kill (&optional append)
  "Kill gtags search history item.

When APPEND, kill appended item(s)."
  (interactive "P")
...
NOCONFIRM when with a prefix arg."
  (interactive "P")
...
(defun ggtags-view-search-history-action ()
  "Gtags view search history action."
  (interactive)
...
(defun ggtags-view-search-history ()
  "Pop to a buffer to view or re-run past search.
  ...
  (let ((split-window-preferred-function ggtags-split-window-function)
    ...
    (cl-labels ((prop (s)
      ...
      (setq ggtags-global-search-ewoc
            (ewoc-create
             #'pp
             "Global search history keys:  n:next  p:prev  r:register  RET:choose\n")))
   ...
   ((prn (data)
     (pcase data
      (`(,command ,root ,_env ,line ,_)
       (princ
        (format
         "a ggtags search session `%s' in directory `%s' at line %d."
         command root line))))))
...
(defun ggtags-make-bookmark-record ()
  "Make current search as a bookmark record."
...
(defun ggtags-bookmark-jump (bookmark)
  "Jump to gtags recorded BOOKMARK."
  (ggtags-global-rerun-search (bookmark-prop-get bookmark 'ggtags-search)))

(defun ggtags-browse-file-as-hypertext (file line)
  "Browse FILE specified LINE in hypertext (HTML) form."
...
If with ARG, go to the corresponding one."
  (interactive)
...
(defvar ggtags-view-tag-history-mode-map
  (let ((keymap (make-sparse-keymap)))
    (define-key keymap (kbd "M-n") #'next-error-no-select)
    (define-key keymap (kbd "M-p") #'previous-error-no-select)
    (define-key keymap (kbd "q")   #'ggtags-kill-window)
    keymap))
...
(defun ggtags-view-tag-history-next-error (&optional arg reset)
  "Go to next tag history error.
When RESET is nil, move to ARGth button."
  (if (not reset)
      (forward-button arg)
    (goto-char (point-min))
...
(defun ggtags-global-exit-message-1 ()
  "Get the total of match and db file used."
  (save-excursion
    (goto-char (point-max))
...
Show EXIT-STATUS and MSG when exit."
...
(defun ggtags-abbreviate-file (start end)
  "Abbreviate single file for START to END."
  (let ((inhibit-read-only t)
        ...
        (advance-word (lambda ()
                        ...
                        (let ((wend (min end (progn (forward-word 1)
                                                    (point))))
...
(defun ggtags-abbreviate-files (start end)
  "Abbreviate files from START to END."
  (goto-char start)
    ...
    (when (and ggtags-global-abbreviate-filename error-re)
      (while (re-search-forward (car error-re) end t)
        (when (and (or (not (numberp ggtags-global-abbreviate-filename))
                   ...
                   (get-text-property
                    (match-beginning sub) 'compilation-message))
...
(defun ggtags-global--display-buffer (&optional buffer desired-point)
  "Display global BUFFER at DESIRED-POINT."
...
Handle exit and show HOW in BUF."
  (cond
   ...
   (ggtags-auto-jump-to-match
    (if (pcase (compilation-next-single-property-change
      ...
      ;;
      ;; - https://debbugs.gnu.org/13829
      ;; - https://debbugs.gnu.org/23987
...
;; `emulation-mode-map-alists', which creates double entries.
(defvar ggtags-navigation-map
  (let ((keymap (make-sparse-keymap)))
    (define-key keymap (kbd "M-n") #'next-error)
    (define-key keymap (kbd "M-p") #'previous-error)
    (define-key keymap (kbd "M-}") #'ggtags-navigation-next-file)
    (define-key keymap (kbd "M-{") #'ggtags-navigation-previous-file)
    (define-key keymap (kbd "M-=") #'ggtags-navigation-start-file)
    (define-key keymap (kbd "M->") #'ggtags-navigation-last-error)
    (define-key keymap (kbd "M-<") #'first-error)
    ;; Note: shadows `isearch-forward-regexp' but it can still be
    ;; invoked with `C-u C-s'.
    (define-key keymap (kbd "C-c M-s") #'ggtags-navigation-isearch-forward)
    ;; Add an alternative binding because C-M-s is reported not
    ;; working on some systems.
    (define-key keymap (kbd "M-s s") #'ggtags-navigation-isearch-forward)
    (define-key keymap (kbd "C-c C-k")
      (lambda () (interactive)
        (ggtags-ensure-global-buffer (kill-compilation))))
    (define-key keymap (kbd "M-o") #'ggtags-navigation-visible-mode)
    (define-key keymap [return] #'ggtags-navigation-mode-done)
    (define-key keymap (kbd "r") 'ggtags-navigation-mode-done)
    (define-key keymap [remap xref-pop-marker-stack] 'ggtags-navigation-mode-abort)
    keymap))
...
(defun ggtags-navigation-mode-cleanup (&optional buf kill)
  "Clean up navigation.
When KILL is non-nil, kill current BUF or last global buffer."
...
(defun ggtags-navigation-mode-done ()
  "Disable `ggtags-navigation-mode'."
  (interactive)
  (ggtags-navigation-mode -1)
...
(defun ggtags-navigation-next-file (n)
  "Navigate to next Nth file."
  (interactive "p")
  (ggtags-ensure-global-buffer
    (compilation-next-file n)
    (compile-goto-error)))

(defun ggtags-navigation-previous-file (p)
  "Navigate to previous Pth file."
  (interactive "p")
  (ggtags-navigation-next-file (- p)))

(defun ggtags-navigation-start-file ()
  "Move to the file where navigation session start."
  (interactive)
...
(defun ggtags-navigation-isearch-forward (&optional regexp-p)
  "Search forward using `isearch'.
When REGEXP-P, support regualr expression."
  (interactive "P")
  (ggtags-ensure-global-buffer
...
(defun ggtags-navigation-visible-mode (&optional arg)
  "Toggle `ggtags-navigation-visible-mode'.
         ...
         (pcase (or ggtags-global-exit-info '(0 0 ""))
           (`(,exit ,count ,db)
            `((:propertize ,(pcase db
              ...
              ,(unless (zerop exit)
                 `(":" (:propertize ,(number-to-string exit)
                                    face error)))))))))
    "]")
...
(defun ggtags-kill-file-buffers (&optional interactively)
  "Kill all buffers visiting files INTERACTIVELY in current project."
  (interactive "p")
  (ggtags-check-project)
...
(cl-defun ggtags-fontify-code (code &optional (mode major-mode))
  "Fontify CODE according to MODE."
...
(defun ggtags-show-definition (name)
  ...
  (let* ((re (cadr (assq 'grep ggtags-global-error-regexp-alist-alist)))
    ...
    (ggtags-with-current-project
      (ggtags-global-output buffer
                            (cons (ggtags-program-path "global") args)
                            show 100))))

(defvar ggtags-mode-prefix-map
  (let ((keymap (make-sparse-keymap)))
    ;; Globally bound to `M-g p'.
    ;; (define-key keymap (kbd "M-'") 'previous-error)
    (define-key keymap (kbd "M-DEL") #'ggtags-delete-tags)
    (define-key keymap (kbd "M-p") #'ggtags-prev-mark)
    (define-key keymap (kbd "M-n") #'ggtags-next-mark)
    (define-key keymap (kbd "M-f") #'ggtags-find-file)
    (define-key keymap (kbd "M-o") #'ggtags-find-other-symbol)
    (define-key keymap (kbd "M-g") #'ggtags-grep)
    (define-key keymap (kbd "M-i") #'ggtags-idutils-query)
    (define-key keymap (kbd "M-b") #'ggtags-browse-file-as-hypertext)
    (define-key keymap (kbd "M-k") #'ggtags-kill-file-buffers)
    (define-key keymap (kbd "M-h") #'ggtags-view-tag-history)
    (define-key keymap (kbd "M-j") #'ggtags-visit-project-root)
    (define-key keymap (kbd "M-/") #'ggtags-view-search-history)
    (define-key keymap (kbd "M-SPC") #'ggtags-save-to-register)
    (define-key keymap (kbd "M-%") #'ggtags-query-replace)
    (define-key keymap (kbd "M-?") #'ggtags-show-definition)
    keymap))

(defvar ggtags-mode-map
  (let ((keymap (make-sparse-keymap))
        (menu (make-sparse-keymap "Ggtags")))
    (define-key keymap [remap xref-find-definitions] #'ggtags-find-tag-dwim)
    (define-key keymap [remap xref-find-references] #'ggtags-find-reference)
    (define-key keymap [remap xref-find-apropos] #'ggtags-find-tag-regexp)
    (define-key keymap ggtags-mode-prefix-key ggtags-mode-prefix-map)
    ;; Menu items
    ...
    (define-key menu [eldoc-mode]
      '(menu-item "Toggle eldoc mode" eldoc-mode
...
(defvar ggtags-mode-line-project-keymap
  (let ((keymap (make-sparse-keymap)))
    (define-key keymap [mode-line mouse-1] 'ggtags-visit-project-root)
    keymap))
...
(define-minor-mode ggtags-mode nil
  ...
  (if ggtags-mode
      (progn
        ...
        ;; Work around https://debbugs.gnu.org/19324
...
(defvar ggtags-highlight-tag-map
  (let ((keymap (make-sparse-keymap)))
    ...
    ;; similar workaround.
    (define-key keymap [S-mouse-1] 'ggtags-find-tag-dwim)
    (define-key keymap [S-down-mouse-1] 'ignore)
    (define-key keymap [S-mouse-3] 'ggtags-find-reference)
    (define-key keymap [S-down-mouse-3] 'ignore)
    keymap)
...
(defun ggtags-setup-highlight-tag-at-point (flag)
  "Setup highlight FLAG for the tag at point."
...
(defun ggtags-cancel-highlight-tag-at-point (&optional keep)
  "Cancel hightlight at point.
When with a prefix arg, use the prefix value to set the KEEP time."
  (when (and (not keep)
                ...
                (let* ((ggtags-print-definition-function
                  ...
                  (condition-case err
                    ...
                    (file-error
                     (remove-function (local 'eldoc-documentation-function)
                                      'ggtags-eldoc-function)
                     (message "Function `ggtags-eldoc-function' disabled for eldoc in current buffer: %S" err)))
                  nil))))))

;;; imenu

(defun ggtags-goto-imenu-index (name line &rest _args)
  "Go to tag according to NAME and LINE."
  (ggtags-forward-to-line line)
...
When OLD string existed, reset string instead init string."
...
(cl-defmethod xref-backend-identifier-at-point ((_backend (eql ggtags)))
  "Implement `xref-backend-identifier-at-point'."
  (ggtags-tag-at-point))

(cl-defmethod xref-backend-identifier-completion-table ((_backend (eql ggtags)))
  "Implement `xref-backend-identifier-completion-table'."
  ggtags-completion-table)
